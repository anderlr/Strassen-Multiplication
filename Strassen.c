//Esse algoritmo implementa o método de Strassen para a multiplicação de matrizes acima de 32x32
...
pixel **strassen_multi(int n, pixel **imagem, pixel **filtro)
{
    ...
    else
    {
        ...
        p1 = strassen_multi(m, a, subtr(m, f, h));
        p2 = strassen_multi(m, soma(m, a, b), h);
        p3 = strassen_multi(m, soma(m, c, d), e);
        p4 = strassen_multi(m, d, subtr(m, g, e));
        p5 = strassen_multi(m, soma(m, a, d), soma(m, e, h));
        p6 = strassen_multi(m, subtr(m, b, d), soma(m, g, h));
        p7 = strassen_multi(m, subtr(m, a, c), soma(m, e, f));
        //junta
...
int main()
{
    ...
    for (int i = 0; i < n; i++) //liberando a memória da imagem, filtro e resultado
    {
        free(imagem[i]);
        free(filtro[i]);
        free(resultado[i]);
